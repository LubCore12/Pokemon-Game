...
def characer_importer(cols, rows, *path):
...
def all_character_import(*path):
	...
	for _, _, image_names in walk(join(*path)):
		for image in image_names:
			...
			new_dict[image_name] = characer_importer(4, 4, *path, image_name)
...
def check_connection(radius, entity, target, tolerance=30):
	...
	if relation.length() < radius:
		if entity.state == 'left' and relation.x < 0 and abs(relation.y) < tolerance or \
		   		entity.state == 'right' and relation.x > 0 and abs(relation.y) < tolerance or \
		   		entity.state == 'up' and relation.y < 0 and abs(relation.x) < tolerance or \
		   		entity.state == 'down' and relation.y > 0 and abs(relation.x) < tolerance:
			return True
